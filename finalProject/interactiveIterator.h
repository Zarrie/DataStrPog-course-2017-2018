...
class interactiveIterator{
	private:
		...
		map currMap;
/*
		std::vector<crossroadName> findPath(const crossroadName &initCR, const crossroadName &destCr) const{
			std::set<crossroadName> visited;
			std::queue<crossroadName> bfsQueue;
			visited.insert(initCR);
			bfsQueue.push(initCR);
			crossroadName currCR;
			while(!bfsQueue.empty()){
				currCR = bfsQueue.front(); bfsQueue.pop();
